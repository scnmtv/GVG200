...
void readingButtonsAndSendingCommands() {
    ...
    if (BUSselect==0)  {
      ...
      for (uint8_t i=1;i<=8;i++)  {
        if (programSelect.isButtonIn(i, busSelection))  { AtemSwitcher.setAuxSourceInput(BUSselect-1, inputs[i-1]); }  // When selecting AUX bus, input 8 is mapped to PROGRAM instead
      }
    } else if (BUSselect==10 || BUSselect==11)  {
      for (uint8_t i=1;i<=8;i++)  {
        if (programSelect.isButtonIn(i, busSelection))  { AtemSwitcher.setMediaPlayerSourceStillIndex(BUSselect-10, shiftState?i+7:i-1); }
      }
    } 
