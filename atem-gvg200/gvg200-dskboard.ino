...
//  Serial.println("Debug: Zagnana funckcija readPreviewKeys");
  for(uint8_t column=0;column<16;column++){
    izhod = boardDSK + column;
    PORTC = izhod; delay(0.1);
    
    pressedKey = PINA;
    if((keyDSKIsDown[column] == true) && (pressedKey == 0b11111111)){
      keyDSKIsDown[column] = false;
      Serial.println(PORTC, BIN);
    } else 
    if ((pressedKey != 0b11111111) && (keyDSKIsDown[column] == false)){
      keyDSKIsDown[column] = true;
      if (serialDebugALL && serialDebugDSK) {
      Serial.println(F("########## DSK Board KEY ###########"));
      ...
      if (serialDebugALL && serialDebugDSK) {
        Serial.print(F("Pressed key in column in DSK Board matrix (values 0-7):  "));
